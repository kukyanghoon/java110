...
public class AppConfig {
    ...
    // 만약 트랜잭션 이름을 다르 ㄴ이름으로 지었다면
    // 트랜잭션 관리 설정에서 그 이름을 알려줘야 한다.
    public PlatformTransactionManager transactionManager(DataSource dataSource) {// ioC컨테이너한테 dataSource달라고 하자  (파라미터로)
        
        // 트랜잭션 관리자가 하는 일은 DB 커넥션의 commit과 rollback을 다루는 것이다.
        // 따라서 트랜잭션 관리자는 DB커넥션을 제공해주는 
        // DataSource(DB 커넥션 풀)가 필요하다.
        // 그래서 트랜잿션 관리자를 만들 떄 반드시 DataSource 객체를 넘겨줘야 한다.
        // 물론 관리자 객체를 만흔 후에 세터를 호출해서 넘겨줘도 된다.
