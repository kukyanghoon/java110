...
/* GET요청
 * => GET요청으로 서버를 값에 보내기 위해서는 Request-URI에 포함해야한다.
...
/* GET요청의 특징
 * - URL에 데이터를 포함시킨다.
 *  => 즐겨찾기에 데이터를 포함하여 URL을 저장할 수 있따.
 *  => 그래서 링크를 누르면 결과 화면으로 바로 갈 수 있다. 
 *  => 보통 조회화면을 요청할 때 GET방식을 사용한다.
 *  
 *  -보안
 *      => URL에 데이터가 포함되어 있지 않기 때문에 데이터가 노출되지 말아야 할 경우
 *         예를 들면 로그인이나 회원가입 등에는 적합하지 않다.
 *  - 대용량 데이터
 *      => 대부분의 웹 서버는 request-uri또는 헤더를 포함한 메타 데이터의 크기를
 *         8KB미만으로 설정하고있다.
 *      => GET요청으로는 대용량의 데이터를 전송할 수 없다.
 *      
 *  - 바이너리 데이터 전송
 *      => request-uri에 포함할 수 있는 것은 텍스트이기 때문에 바이너리 데이터(이미지,엑셀,영상 등)를 전송하는데 적합하지 않다.
 *      => 물론 Base64 인코딩을 이용하여 바이너리 데이터를 텍스트 데이터로 변환한 다음에 보낼 수는 있다.
 *      => 하지만, 결국 대용량의 데이터인 경우에는 마찬가지고 크기 제한때문에 불가능하다.
 *      
 *      
 *  - 적용 부분
