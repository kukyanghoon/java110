...
public class Test22 {
     ...
     * 프론트 컨트롤러는 request handler의 리턴 값을 클라이언트로 바로 출력하다.
     * 1) 리턴 타입이 String 과 primitive type일 경우,
     *    => 문자열로 변환한 후 출력한다.
     * 2) 리턴 타입이 기타 객체일 경우,
     *    => HttpMessageConverter 규칙에 따라 만든 클래스를 찾아
     *       그 클래스를 이용하여 리턴한 객체의 값을 JSON 형식의 문자열로 변환한 후 
     *       출력한다.
     *    => "Http Message"는 클라이언트로부터 받은 데이터,
     *       클라이언트로 보내는 데이터를 가리킨다.
     *    => 만약 HttpMessageConverter 구현체를 못 찾았다면?
     *       리턴 값을 바꿀 수 없기 때문에 실행오류가 발생한다.
     *       
     * HttpMessageConverter 구현체는 어떻게 추가하는가?
     * => 스프링 프레임워크는 기본 데이터 변환기로 다음 두 개의 클래스를 사용하도록 코딩해 두었다.
     ...
     *          "google-gson"라이브러리
     * 
     * => 따라서 @RestController를 사용하려면 위의 두 라이브러리중 하나를
     *    반드시 포함해야 한다.
     */
     ...
     *
     * 방법1) 클라이언트가 보낸 JSON데이터를 변수에 받아서 페이지 컨트롤러가 처리하기
    ...
    // produces속성에 어떤 콘텐트를 리턴할 지 그 타입을 지정하지 않는다면,
    ...
    // @RestController일 경우 리턴 타입이 primitive type이라면
    // 문자열로 변환하여 그대로 출력한다.
    @RequestMapping("m5")
    ...
    //클라이언트가 보낸 JSON 데이터를 String변수에 받기
    // => String -> String
    @RequestMapping("m6")
