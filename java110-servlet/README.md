...
[이클립스 작업 폴도] 예) C:\Users\bit\workspace

.metadata\
    .plugins\
        org.eclipse.wst.server.core\
            tmp0\           - 톰캣 테스트 환경 폴더(생성된 순서대로 0,1,2... 식으로 이름이 부여된다.)
                conf\       - 이클립스 "Servers/테스트환경폴더"에 별도로 편집된 설정파일을 복사해 온다.
...
###애플리케이션 배포
- 톰캣 서버 테스트 환경 메뉴(Servers뷰에 등록한 테스트환경)에서 "Add and Remove..."선택
- 왼쪽에 나열된 프로젝트 중에서 테스트할 프로젝트를 선택한 후 add한다.
- 톰캣 서버 테스트 환경에서 "Publish"메뉴 선택 또는 "Start"를 선택하면 된다.
...
-톰캣 서버 테스트 환경에는 톰캣 서버 원본에 있던 웹애플리케이션이 없기 떄문에 기본으로 출력하는 웹 페이지가 없다.
...
배치폴더 (tmp0/wtpwebapps)에 프로젝트를 배치할 때 프로젝트 이름으로 배치 되더라도
...
- 톰캣 서버 테스트 환경목록에서 해당 서버를 더블클릭한다
- 화면에서 modules 탭을 선택한다.
- 웹 애플리케이션을 선택하고 edit한다.
- "Path"의 값을 "/"로 설정한다.
    -실제는 server.xml파일에 배치 정보를 변경하는 것이다.
-톰캣 서버를 다시 시작한다.
...
-컴포넌트
    - 서블릿, 필터, 리스너
    - 컴포넌트는 배치될 때 /WEB-INF/classes 폴더에 패치된다.
...
-HTTP Server
    ...
    - 보통 실무에서 운영할 때는 웹 애플리케이션 서버에 내장된 HTTP Server을 사용하지 않고
    별도의 HTTP Server(ex: Apache, nginX등)를 사용한다.
...
- EJB Container
    ...
    - 현재는 EJB를 잘 사용하지 않는다.
    - 왜? POJO 기반의 Spring Framework를 주로 사용하기 때문이다.
...
- 주요 기술 요소
    ...
    - 분산 컴포넌트 기술 : EJB, Message Service, Transation, JavaMail등
    - 웹 서비스 기술 : JAX-PS, JAX-PRC, JAXR등
    - 관리 및 보안 기술
...
-Java EE 버전
    ...
    - 버전 예:
        - Java EE 8 : Servlet 4.0, JSP 2.3, JSTL 1.2, EJB 3.2등
        - Java EE 7 : Servlet 3.1, JSP 2.3, JSTL 1.2, EJB 3.2등
        - Java EE 6 : Servlet 3.0, JSP 2.2, JSTL 1.2, EJB 3.1등
        - Java EE 5 : Servlet 2.5, JSP 2.1, JSTL 1.1, EJB 3.0등

- 웹 애플리케이션을 제작할 때 주의할 사항!
    - WAS 제품의 버전을 확인하라!
    - 그 제품이 Java EE 어떤 버전의 구현체인지 확인하라!
    - 그 버전에 맞춰서 Servlet/JSP 분법을 사용하라!
