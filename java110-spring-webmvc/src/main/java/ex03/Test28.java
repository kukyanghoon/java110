/* 세션 다루기 - @ModelAttributes 애노테이션과 SessionStatus
 */
package ex03;
...
// request handler가 Model이나 Map객체에 저장하는 값 중에서
// 세션에도 저장할 값을 지정하는 애노테이션이다.
...
public class Test28 {
    ...
    public String m1(Model model, HttpSession session) throws Exception{
        // 프론트 컨트롤러가 ServletRequest에 값 보관 + 세션에도 보관함,
        // => @SessionATtributes에 지정된 이름이기 때문에
        model.addAttribute("name2","홍길순"); // 프론트 컨트롤러가 세션에 보관
    ...
    @ResponseBody
    public String m2(
            // @SessionAttributes에 지정된 값을 받고 싶을때
    ...
    public String m3(HttpSession session) throws Exception{
        ...
        String tel = (String)session.getAttribute("tel2");
        String gender = (String)session.getAttribute("gender2");
        String age = (String)session.getAttribute("age2");
        return String.format("%s, %s, %s, %s\n", name, tel, gender, age);
    }
    ...
    public String m4(
            ...
            // 지정한 값을 받을 때?
            // => 안된다. 현재 페이지 컨트롤러에서 지정된 이름의 값만 받을 수 있다.
            @ModelAttribute("name") String name,
            @ModelAttribute("tel2") String tel,
    ...
    public String logout2(SessionStatus status) throws Exception{
     // 현재 이 request handler가 소속되어 있는 페이지 컨트롤러에서
        ...
        //=> Test27 페이지 컨트롤러에엇 관리되는 name,gender는 아니다.
        // => 기존의 HttpSession 객체는 계속 유효하다.
        // => HttpSession객체에 직접 저장한 값도 계속 유효하다.
        status.setComplete();
        return "세션 무효화함ㅋ";
    }
}
...
//    "페이지 컨트롤러가 작업을 수행하기 위해 잠시 세션을 이요했는데,
//    작업이 완료되어서 세션에 잠시 보관된 값들을 제거하고 싶다."라는 의미이다.
//    
